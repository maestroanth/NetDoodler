...
(function($) {
    ...
    $slider.mousemove(function(e){ // Get the cursor position
        ...
        if($sldr.hasClass('multi-slides')){
            ...
            if($mousePos.x > $(window).width()/2) {
                $sldr.css('cursor', 'url(/modules/tera-slider/slide-right-dark.png), e-resize');
                if($('body').hasClass('dark-hero'))
                    $sldr.css('cursor', 'url(/modules/tera-slider/slide-right.png), e-resize');
            } else {
               $sldr.css('cursor', 'url(/modules/tera-slider/slide-left-dark.png), w-resize');
                if($('body').hasClass('dark-hero'))
                    $sldr.css('cursor', 'url(/modules/tera-slider/slide-left.png), w-resize');
            }
        }
    });
    ...
    function toggleSlide(e,d) {
        ...
        function animateSlide(){
          if($transition == 'slide' && $slideCount != 1){
            ...
            $activeSlide.next().css({ 'transform': 'scale(1)'});
            $activeSlide.prev().css({ 'transform': 'scale(1)'});
            sliderResize();
          }
          if($transition == 'fade' && $slideCount != 1){
